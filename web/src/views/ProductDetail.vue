  ...
  <div class="max-w-7xl mx-auto px-6 py-8">
    <div v-if="product.id" class="flex flex-col lg:flex-row gap-10">
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="bg-gradient-to-r from-red-50 to-orange-50 rounded-xl px-6 py-4 mb-6">
          <div class="flex items-baseline gap-3">
            ...
            <span v-if="product.origin_price"
              ...
              {{ $t('productDetail.save') }}¥{{ (product.origin_price - (selectedSku?.price ?? product.price)).toFixed(0) }}
            </span>
          </div>
          <div class="flex gap-4 mt-2 text-xs text-gray-500">
            <span>{{ $t('productDetail.stock') }} {{ selectedSku?.stock ?? product.stock }}</span>
            <span>{{ $t('productDetail.sold') }} {{ product.sales }}</span>
            <span>{{ $t('productDetail.favCount') }} {{ product.favorite_count || 0 }}</span>
          </div>
        </div>
        ...
        <!-- Quantity -->
        <div class="mb-8">
          <h3 class="text-sm font-semibold text-gray-700 mb-3">{{ $t('productDetail.quantity') }}</h3>
        ...
        <div class="flex gap-3">
          ...
          <button @click="toggleFavorite" class="btn-outline !py-3 flex-center gap-2 w-36">
            ...
            {{ product.is_favorited ? $t('productDetail.favorited') : $t('productDetail.favorite') }}
          </button>
        ...
        <div class="mt-10 pt-8 border-t border-gray-100">
          ...
          <div v-if="activeTab === 'detail'" class="space-y-4">
            ...
            <p v-if="!detailBlocks.length" class="text-sm text-gray-400">{{ $t('productDetail.noDetail') }}</p>
          </div>

          <div v-else class="space-y-4">
            <div class="bg-gray-50 rounded-xl p-4">
              <div class="flex-between text-sm text-gray-600 mb-1">
                <span>{{ $t('productDetail.productScore') }}</span><span class="font-semibold text-red-500">{{ reviewSummary.avg_product_score.toFixed(1) }}</span>
              </div>
              <div class="flex-between text-sm text-gray-600">
                <span>{{ $t('productDetail.logisticsScore') }}</span><span class="font-semibold text-red-500">{{ reviewSummary.avg_logistics_score.toFixed(1) }}</span>
              </div>
            </div>
            ...
            <div v-if="reviews.length" class="space-y-3">
              <div v-for="rv in reviews" :key="rv.id" class="border border-gray-100 rounded-xl p-4">
                <div class="flex-between mb-2">
                  <p class="text-sm font-medium text-gray-800">{{ rv.user_nickname || $t('productDetail.anonymousUser') }}</p>
                  <p class="text-xs text-gray-400">{{ formatDate(rv.created_at) }}</p>
                </div>
                <p class="text-xs text-gray-500 mb-2">{{ $t('productDetail.product') }} {{ rv.product_score }} / {{ $t('productDetail.logistics') }} {{ rv.logistics_score }}</p>
                <p class="text-sm text-gray-600 leading-6">{{ rv.content || $t('productDetail.noReviewContent') }}</p>
                ...
                <div v-if="rv.appends?.length" class="mt-3 bg-gray-50 rounded-lg p-3">
                  <div v-for="ap in rv.appends" :key="ap.id" class="mb-2 last:mb-0">
                    <p class="text-xs text-gray-500">{{ $t('productDetail.appendPrefix') }}{{ ap.content || $t('productDetail.imageOnlyAppend') }}</p>
...
const tabs = computed<Array<{ label: string; value: 'detail' | 'review' }>>(() => [
  ...
  { label: t('productDetail.tabReview'), value: 'review' },
])
