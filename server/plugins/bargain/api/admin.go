...
func RegisterAdminRoutes(r *gin.RouterGroup) {
	g := r.Group("/admin/api/bargain")
	{
		// 活动管理
		g.GET("/activities", api.RequirePermission("bargain:view"), listActivities)
		g.POST("/activities", api.RequirePermission("bargain:edit"), createActivity)
		g.PUT("/activities/:id", api.RequirePermission("bargain:edit"), updateActivity)
		g.DELETE("/activities/:id", api.RequirePermission("bargain:delete"), deleteActivity)

		// 商品管理
		g.GET("/products", api.RequirePermission("bargain:view"), listProducts)
		g.PUT("/activities/:id/products", api.RequirePermission("bargain:edit"), upsertProducts)

		// 砍价订单管理
		g.GET("/orders", api.RequirePermission("bargain:view"), listBargainOrders)
		g.GET("/orders/:id", api.RequirePermission("bargain:view"), getBargainOrder)
		g.GET("/orders/:id/helpers", api.RequirePermission("bargain:view"), listBargainHelpers)
	}
}
...
func listBargainOrders(c *gin.Context) {
	page, _ := strconv.Atoi(c.DefaultQuery("page", "1"))
	size, _ := strconv.Atoi(c.DefaultQuery("size", "20"))
	status := c.Query("status")

	orders, total, err := service.ListBargainOrders(c.Request.Context(), page, size, status)
	if err != nil {
		api.Error(c, err)
		return
	}
...
func listBargainHelpers(c *gin.Context) {
	id, _ := strconv.ParseUint(c.Param("id"), 10, 64)
