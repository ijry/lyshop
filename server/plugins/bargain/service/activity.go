...
func ListActivities(ctx context.Context, page, size int, status *int8) ([]model.BargainActivity, int64, error) {
	var activities []model.BargainActivity
	var total int64

	query := db.DB.WithContext(ctx).Model(&model.BargainActivity{})
	if status != nil {
...
// GetActivity 获取活动详情
func GetActivity(ctx context.Context, id uint64) (*model.BargainActivity, error) {
...
// CreateActivity 创建活动
func CreateActivity(ctx context.Context, activity *model.BargainActivity) error {
	// 检查时间冲突
	if activity.StartAt != nil && activity.EndAt != nil {
		var count int64
		err := db.DB.WithContext(ctx).Model(&model.BargainActivity{}).
			Where("status = 1").
...
func UpdateActivity(ctx context.Context, id uint64, updates map[string]interface{}) error {
	return db.DB.WithContext(ctx).Model(&model.BargainActivity{}).Where("id = ?", id).Updates(updates).Error
}
...
func DeleteActivity(ctx context.Context, id uint64) error {
	return db.DB.WithContext(ctx).Transaction(func(tx *db.Tx) error {
		...
		// 删除活动
		return tx.Delete(&model.BargainActivity{}, id).Error
	})
}

// GetActiveActivities 获取当前有效的活动
func GetActiveActivities(ctx context.Context) ([]model.BargainActivity, error) {
	var activities []model.BargainActivity
	now := time.Now()
