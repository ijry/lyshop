...
docs-site 首页会使用演示模式自动截图展示主要功能界面，覆盖：

- H5 商城首页：装修组件、营销入口、推荐商品。
- H5 商品列表：分类筛选与商品流。
- H5 订单列表：订单状态、售后与履约入口。
- PC 商城首页：桌面端商城首屏与商品展示。
...
生成图片位于 `docs/public/showcase/`，由 VitePress 作为静态资源发布。该能力只影响文档站构建流程，不改变后端接口、业务插件或生产部署配置。
