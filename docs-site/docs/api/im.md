...
| 接口 | 权限 | 说明 |
|---|---|---|
| `GET /api/v1/im/session` | 用户登录 | 获取或创建当前用户开放会话，支持上报访客上下文，返回 `mode`：`ai`/`human` |
...
`GET /api/v1/im/session` 可携带以下可选查询参数，Web、H5 或嵌入脚本用于把当前访问环境同步给客服工作台：

| 参数 | 说明 |
|---|---|
| `visitor_id` | 站点侧访客标识，可由嵌入脚本生成并持久化 |
| `visitor_location` | 访客地区，业务侧可传入 IP 解析结果 |
...
| `visitor_device` | 设备类型，如 `desktop`、`mobile` |
| `visitor_extra` | JSON 字符串，保存页面标题、UA、渠道等扩展信息 |

后端会同时记录请求 IP 到 `visitor_ip`。客服端收到 `assign` 帧时，`payload.visitor` 会包含上述安全字段；后台会话列表接口也返回这些字段，便于坐席判断用户来源和上下文。

## Web 嵌入脚本

Web 端提供 `web/public/im-widget.js`，部署后可通过 `/im-widget.js` 引入任意站点。脚本以 iframe 方式打开现有 `/chat?embed=1` 页面，不新增客服前端应用。
...
<script>
  LYShopIMWidget.init({
    ...
    context: {
      visitor_location: '上海',
      visitor_extra: { channel: 'landing-page' }
    }
  })
</script>
```

脚本会自动采集 `visitor_language`、`visitor_referrer`、`visitor_url`、`visitor_device`、页面标题和 UA，并通过 `postMessage` 发送给 iframe。嵌入站点必须向脚本传入可用用户 JWT，或在业务侧改造为登录后展示客服入口。
...
| `typing_draft` | 双向转发 | 用户或客服正在输入的草稿预览 |
| `typing_stop` | 双向转发 | 输入停止或已发送 |
...
服务端会把草稿截断到 500 个字符，并只转发给当前会话对端。发送消息或清空输入框时发送 `typing_stop`。
...
事件日志记录 `event`、`level`、`category`、`trace_id`、`session_id`、`user_id`、`staff_id`、`message_id`、`source`、`success`、`latency_ms`、`message`、`meta`、`extra`，用于审计、排障和报表统计。日志查询支持 `level`、`category`、`trace_id` 和 `keyword` 过滤，`keyword` 会匹配 `event/message`。
...
- **可选联网搜索**：`ai_web_search_enabled` 默认关闭。开启并配置 Serper API Key 后，AI 会在回答前按检索问题获取外部搜索摘要，作为 `【联网搜索】` 上下文注入提示词，并记录 `web_search` 日志；搜索失败不阻断 AI 回复。
...
- 联网搜索需要后端能访问 `ai_web_search_endpoint`，并配置 `ai_web_search_api_key`；未配置或关闭时不会发起外部请求。
