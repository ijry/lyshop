...
- **检索增强（RAG）**：召回优先走 Qdrant 向量库（配置 `ai_qdrant_url` + 向量模型，ANN 检索、`status=1` 过滤、可选相似度阈值，按命中 ID 回查并保序，可扩展到大规模知识库）；仅配置向量模型时退化为内存余弦；均未配置时关键词召回。知识条目在「AI知识库」页面维护，CRUD/导入/重建会与 Qdrant 双写同步。`docker-compose.yml` 已内置 `qdrant` 服务（容器内地址 `http://qdrant:6333`）。
