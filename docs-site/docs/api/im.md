...
- **检索增强（RAG）**：完整 recall→fuse→rerank pipeline。召回优先走 Qdrant ANN；开启 `ai_hybrid` 时向量 + 关键词双路召回经 RRF 融合；配置 `ai_rerank_url` 时 cross-encoder 精排候选池至 TopK（兼容 Cohere/Jina/TEI `/rerank`）。各阶段均可独立关闭，未配置时依次退化为内存余弦→关键词。知识条目在「AI知识库」页面维护，CRUD/导入/重建会与 Qdrant 双写同步。`docker-compose.yml` 已内置 `qdrant` 服务（容器内地址 `http://qdrant:6333`）。
