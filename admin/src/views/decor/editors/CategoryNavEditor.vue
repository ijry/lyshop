<template>
  <div class="space-y-4">
    <div class="flex items-center justify-between">
      <label class="text-xs text-slate-500">导航项</label>
      ...
      <div v-for="(item, idx) in modelValue.items" :key="idx"
        ...
        <div>
          ...
          <input :value="item.title" @input="updateItem(idx, 'title', ($event.target as HTMLInputElement).value)"
            placeholder="如：手机数码" class="w-full border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs focus:outline-none focus:border-blue-400" />
        </div>

        <ImageUpload :modelValue="item.icon" @update:modelValue="updateItem(idx, 'icon', $event)" label="图标" />
        <LinkPicker :modelValue="item.link" @update:modelValue="updateItem(idx, 'link', $event)" label="跳转链接" />
      </div>
    </div>
