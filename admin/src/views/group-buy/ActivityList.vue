  ...
  <div class="page-container">
    ...
    <el-table :data="activities" border stripe>
      ...
      <el-table-column label="成团人数" width="100">
        <template #default="{ row }">{{ row.group_size }}人</template>
      </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="editingId ? '编辑活动' : '新增活动'" width="600px">
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="成团人数" required>
          <el-input-number v-model="form.group_size" :min="2" :max="100" />
        </el-form-item>
        <el-form-item label="过期时间" required>
          <el-input-number v-model="form.expire_hours" :min="1" :max="168" />
...
async function loadActivities() {
  const data = await request.get<any>('/group-buy/activities', {
    params: {
      page: page.value,
      size: size.value,
    }
...
async function handleSave() {
  ...
  try {
    if (editingId.value) {
      await request.put(`/group-buy/activities/${editingId.value}`, form.value)
      ElMessage.success('更新成功')
    } else {
      await request.post('/group-buy/activities', form.value)
...
async function handleDelete(id: number) {
  try {
    ...
    await request.delete(`/group-buy/activities/${id}`)
...
function goToProducts(activityId: number) {
  router.push(`/group-buy/products?activity_id=${activityId}`)
}
