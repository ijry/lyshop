...
async function loadActivities() {
  const data = await get<any>('/admin/api/bargain/activities', {
    page: page.value,
    size: size.value,
  })
...
async function handleSave() {
  ...
  try {
    if (editingId.value) {
      await put(`/admin/api/bargain/activities/${editingId.value}`, form.value)
      ElMessage.success('更新成功')
    } else {
      await post('/admin/api/bargain/activities', form.value)
...
async function handleDelete(id: number) {
  try {
    ...
    await del(`/admin/api/bargain/activities/${id}`)
