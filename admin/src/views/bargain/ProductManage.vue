<template>
  <div class="page-container">
    ...
    <el-table :data="products" border stripe>
      ...
      <el-table-column label="起始价/底价" width="180">
        <template #default="{ row }">
          <div style="display: flex; gap: 5px; align-items: center">
            <el-input-number v-model="row.start_price" :min="0" :precision="2" size="small" style="width: 80px" />
            <span>/</span>
            <el-input-number v-model="row.floor_price" :min="0" :precision="2" size="small" style="width: 80px" />
          </div>
        </template>
      </el-table-column>
      <el-table-column label="砍价范围" width="180">
        <template #default="{ row }">
          <div style="display: flex; gap: 5px; align-items: center">
            <el-input-number v-model="row.min_cut_amount" :min="0.01" :precision="2" size="small" style="width: 80px" />
            <span>~</span>
            <el-input-number v-model="row.max_cut_amount" :min="0.01" :precision="2" size="small" style="width: 80px" />
          </div>
        </template>
      </el-table-column>
      <el-table-column label="最多助力" width="120">
        <template #default="{ row }">
          <el-input-number v-model="row.max_helpers" :min="1" size="small" />
        </template>
      </el-table-column>
      <el-table-column label="过期时间" width="120">
        <template #default="{ row }">
          <el-input-number v-model="row.expire_hours" :min="1" size="small" />
    ...
    <el-dialog v-model="addDialogVisible" title="添加商品" width="600px">
      <el-form :model="addForm" label-width="120px">
        ...
        <el-form-item label="起始价格" required>
          <el-input-number v-model="addForm.start_price" :min="0" :precision="2" style="width: 100%" />
        </el-form-item>
        <el-form-item label="底价" required>
          <el-input-number v-model="addForm.floor_price" :min="0" :precision="2" style="width: 100%" />
        </el-form-item>
        <el-form-item label="最小砍价金额">
          <el-input-number v-model="addForm.min_cut_amount" :min="0.01" :precision="2" style="width: 100%" />
        </el-form-item>
        <el-form-item label="最大砍价金额">
          <el-input-number v-model="addForm.max_cut_amount" :min="0.01" :precision="2" style="width: 100%" />
        </el-form-item>
        <el-form-item label="最多助力人数">
          <el-input-number v-model="addForm.max_helpers" :min="1" style="width: 100%" />
        </el-form-item>
        <el-form-item label="过期时间">
          <el-input-number v-model="addForm.expire_hours" :min="1" style="width: 100%" />
...
async function loadActivities() {
  const data = await get<any>('/admin/api/bargain/activities', { page: 1, size: 100 })
...
async function loadProducts() {
  ...
  const data = await get<any>('/admin/api/bargain/products', {
...
function handleAdd() {
  if (!addForm.value.product_id || addForm.value.start_price <= 0 || addForm.value.floor_price <= 0) {
    ElMessage.error('请填写完整信息')
    return
  }

  if (addForm.value.floor_price >= addForm.value.start_price) {
    ElMessage.error('底价必须小于起始价格')
    return
  }
...
async function handleBatchSave() {
  ...
  try {
    await put(`/admin/api/bargain/activities/${activityId.value}/products`, products.value)
