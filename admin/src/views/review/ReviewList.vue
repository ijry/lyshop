<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h2 class="text-xl font-semibold text-slate-800">评价列表</h2>
    </div>

    <div class="bg-white rounded-xl shadow-sm p-4 mb-4 border border-slate-100 flex gap-3 flex-wrap">
      <input v-model="query.keyword" placeholder="搜索评价内容" class="border border-slate-200 rounded-lg px-3 py-2 text-sm flex-1 min-w-[220px]" />
      <input v-model="query.product_id" type="number" placeholder="商品ID" class="border border-slate-200 rounded-lg px-3 py-2 text-sm w-32" />
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <tbody class="divide-y divide-slate-50">
          <tr v-for="rv in list" :key="rv.id" class="align-top hover:bg-slate-50">
            ...
            <td class="px-4 py-3 text-xs text-slate-500">
              ...
              <p class="mt-1" :class="rv.admin_reply ? 'text-emerald-600' : 'text-slate-400'">{{ rv.admin_reply ? '已回复' : '未回复' }}</p>
            </td>
            <td class="px-4 py-3">
              <button class="text-blue-600 hover:underline text-xs mr-3" @click="openDetail(rv.id)">详情</button>
              <button class="text-emerald-600 hover:underline text-xs" @click="openReply(rv.id)">回复</button>
            </td>
          </tr>
          <tr v-if="!list.length">
      ...
      <div class="px-4 py-3 flex items-center justify-between border-t border-slate-100 text-sm text-slate-500">
        ...
        <div class="flex gap-2">
          <button :disabled="query.page <= 1" @click="prevPage" class="px-3 py-1 rounded-lg border hover:bg-slate-50 disabled:opacity-40">上一页</button>
          <button :disabled="query.page * query.size >= total" @click="nextPage" class="px-3 py-1 rounded-lg border hover:bg-slate-50 disabled:opacity-40">下一页</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="showReply" class="fixed inset-0 bg-black/35 flex items-center justify-center z-50" @click.self="closeReply">
      <div class="bg-white rounded-xl w-[560px] max-w-[92vw] p-6">
        ...
        <textarea v-model="replyContent" class="w-full border border-slate-200 rounded-xl p-3 text-sm min-h-[120px] outline-none focus:border-blue-300" placeholder="请输入回复内容" />
        <div class="flex justify-end gap-2 mt-4">
          <button class="px-4 py-2 border rounded-lg text-sm" @click="closeReply">取消</button>
          <button class="px-4 py-2 bg-blue-700 text-white rounded-lg text-sm disabled:opacity-50" :disabled="replying" @click="submitReply">
            {{ replying ? '提交中...' : '提交回复' }}
          </button>
        </div>
      </div>
...
import { getReviews, getReviewDetail, replyReview } from '@/api/plugins'
...
async function load() {
  const data: any = await getReviews({
    ...
    product_id: query.value.product_id || undefined,
