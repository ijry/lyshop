...
const routes: Record<string, any> = {
    ...
    groups: [
      {
        ...
        menus: [
          { title: '商品管理', icon: 'box', path: '/product', sort: 10, children: [
            ...
            { title: '规格模板', path: '/product/spec-templates' },
  ...
  // IM
  // IM AI 知识库
  'GET /admin/api/im/knowledge': { list: [
    { id: 1, title: '退货政策', content: '支持7天无理由退货，商品须保持完好原状。退货运费由买家承担，商品质量问题由商家承担。申请退货后3个工作日内处理，退款1-3个工作日到账。', tags: '退货,售后,退款', indexed: 1, sort: 0, status: 1, created_at: '2026-05-20T10:00:00Z' },
    { id: 2, title: '发货时效', content: '付款成功后1-2个工作日内发货，节假日顺延。发货后会推送物流信息，可在订单详情页查看实时物流。', tags: '发货,物流,时效', indexed: 1, sort: 1, status: 1, created_at: '2026-05-20T10:05:00Z' },
    { id: 3, title: '支付方式', content: '支持微信支付、支付宝支付和账户余额支付。所有支付渠道均经过安全加密，保障交易安全。', tags: '支付,微信,支付宝', indexed: 1, sort: 2, status: 1, created_at: '2026-05-20T10:10:00Z' },
    { id: 4, title: '优惠券使用规则', content: '优惠券分为满减券、折扣券和无门槛券三种。每笔订单最多使用1张优惠券，不可叠加使用。部分商品不参与优惠券活动。', tags: '优惠券,折扣,活动', indexed: 1, sort: 3, status: 1, created_at: '2026-05-20T10:15:00Z' },
    { id: 5, title: '商品质量保障', content: '所有商品均为正品保障，假一赔十。如收到商品存在质量问题，可申请退换货，运费由商家承担，处理时间不超过3个工作日。', tags: '正品,质量,保障', indexed: 0, sort: 4, status: 1, created_at: '2026-05-20T10:20:00Z' },
    { id: 6, title: '会员积分规则', content: '每消费1元获得1积分，积分可在下次购物时抵扣，100积分=1元。积分有效期为1年，过期自动清零。VIP会员积分双倍。', tags: '积分,会员,VIP', indexed: 0, sort: 5, status: 0, created_at: '2026-05-20T10:25:00Z' },
  ], total: 6, page: 1, size: 20 },
  'POST /admin/api/im/knowledge': { id: 99, title: '新条目', content: '', tags: '', indexed: 0, sort: 0, status: 1, created_at: new Date().toISOString() },
  'PUT /admin/api/im/knowledge/*': { success: true },
  'DELETE /admin/api/im/knowledge/*': { success: true },
  'POST /admin/api/im/knowledge/reindex': { indexed: 5 },
  'POST /admin/api/im/knowledge/import': { filename: 'faq.docx', chunks: 8 },
  'POST /admin/api/im/ai/test': { reply: '连接正常，模型已就绪' },
  'POST /admin/api/im/ai/rerank-test': { reply: '连接正常' },

  // IM 评估反馈
  'GET /admin/api/im/feedback': { list: [
    { id: 1, session_id: 1, source: 'auto', rating: 0, comment: '', faithfulness: 4, relevance: 5, query: '退货政策是什么？', answer: '支持7天无理由退货，商品须保持完好原状。', created_at: '2026-05-28T10:31:00Z' },
    { id: 2, session_id: 1, source: 'user', rating: 1, comment: '解答得很清楚', faithfulness: 0, relevance: 0, query: '发货多久？', answer: '付款成功后1-2个工作日内发货。', created_at: '2026-05-28T10:45:00Z' },
    { id: 3, session_id: 2, source: 'auto', rating: 0, comment: '', faithfulness: 2, relevance: 3, query: '可以用优惠券吗', answer: '您好，请问您想了解哪方面的优惠券问题？', created_at: '2026-05-27T15:20:00Z' },
    { id: 4, session_id: 3, source: 'user', rating: -1, comment: '没回答我的问题', faithfulness: 0, relevance: 0, query: '积分怎么用', answer: '您好，有什么可以帮您的？', created_at: '2026-05-27T16:05:00Z' },
  ], total: 4, page: 1, size: 20 },
  'GET /admin/api/im/feedback/stats': {
    auto: { count: 2, avg_faith: 3.0, avg_relevance: 4.0, avg_rating: null },
    user: { count: 2, avg_faith: null, avg_relevance: null, avg_rating: 0.0 },
  },

  // IM 客服状态与坐席
  'GET /admin/api/im/staff/status': { admin_id: 1, is_online: 1, max_load: 5, current_load: 3 },
  'POST /admin/api/im/staff/online': { success: true },
  'GET /admin/api/im/staff': [
    { id: 1, admin_id: 1, is_online: 1, max_load: 5, current_load: 3 },
    { id: 2, admin_id: 2, is_online: 0, max_load: 5, current_load: 0 },
    { id: 3, admin_id: 3, is_online: 1, max_load: 8, current_load: 5 },
  ],
  'POST /admin/api/im/staff': { success: true },
  'PUT /admin/api/im/staff/*': { success: true },
  'DELETE /admin/api/im/staff/*': { success: true },
  'POST /admin/api/im/sessions/*/accept': { success: true },
  'POST /admin/api/im/sessions/*/close': { success: true },
  'POST /admin/api/im/sessions/*/transfer': { success: true },
  'GET /admin/api/im/sessions': [
    { id: 1, user_id: 1001, user_nickname: '张三', user_avatar: 'https://picsum.photos/80/80?random=u1', staff_id: 1, mode: 'human', status: 2, queue_position: 0, last_msg: '这款耳机降噪效果怎么样？', unread_count: 1, updated_at: '2026-05-28T10:30:00Z' },
    { id: 2, user_id: 1002, user_nickname: '李四', user_avatar: 'https://picsum.photos/80/80?random=u2', staff_id: 0, mode: 'human', status: 1, queue_position: 1, last_msg: '你好，我想退货，订单号 10003', unread_count: 3, updated_at: '2026-05-28T09:15:00Z' },
    { id: 3, user_id: 1003, user_nickname: '王五', user_avatar: 'https://picsum.photos/80/80?random=u3', staff_id: 1, mode: 'human', status: 2, queue_position: 0, last_msg: '物流显示已签收但我没收到', unread_count: 0, updated_at: '2026-05-27T18:42:00Z' },
    { id: 4, user_id: 1004, user_nickname: '赵六', user_avatar: 'https://picsum.photos/80/80?random=u4', staff_id: 1, mode: 'human', status: 3, queue_position: 0, last_msg: '好的，感谢解答', unread_count: 0, updated_at: '2026-05-27T14:20:00Z' },
    { id: 5, user_id: 1005, user_nickname: '陈七', user_avatar: 'https://picsum.photos/80/80?random=u5', staff_id: 0, mode: 'ai', status: 2, queue_position: 0, last_msg: '请问有没有大号的？', unread_count: 1, updated_at: '2026-05-28T11:05:00Z' },
    { id: 6, user_id: 1006, user_nickname: '何八', user_avatar: 'https://picsum.photos/80/80?random=u6', staff_id: 0, mode: 'ai', status: 2, queue_position: 0, last_msg: '发票什么时候能开好？', unread_count: 2, updated_at: '2026-05-28T08:50:00Z' },
    { id: 7, user_id: 1007, user_nickname: '孙九', user_avatar: 'https://picsum.photos/80/80?random=u7', staff_id: 0, mode: 'human', status: 1, queue_position: 3, last_msg: '这个颜色和图片上差别大吗', unread_count: 1, updated_at: '2026-05-28T07:30:00Z' },
  ],
  'GET /admin/api/im/sessions/': { list: [
    { id: 1,  session_id: 1, sender_type: 0, content: 'AI 智能助手已为您服务，如需人工客服请输入「人工」', type: 'system', created_at: '2026-05-28T10:18:00Z' },
    { id: 2,  session_id: 1, sender_type: 1, content: '你好，我想问一下这款耳机', type: 'text', created_at: '2026-05-28T10:20:00Z' },
    { id: 3,  session_id: 1, sender_type: 3, content: '您好！我是 AI 智能助手，很高兴为您服务。请问您想了解哪款耳机的具体信息呢？', type: 'text', created_at: '2026-05-28T10:20:00Z' },
    { id: 4,  session_id: 1, sender_type: 1, content: '这款耳机降噪效果怎么样？', type: 'text', created_at: '2026-05-28T10:22:00Z' },
    { id: 5,  session_id: 1, sender_type: 3, content: '根据商品资料，这款耳机采用主动降噪技术，可有效过滤 90% 以上环境噪音，适合通勤和办公场景使用，用户评价普遍较高。', type: 'text', created_at: '2026-05-28T10:22:00Z' },
    { id: 6,  session_id: 1, sender_type: 1, content: '续航怎么样？', type: 'text', created_at: '2026-05-28T10:25:00Z' },
    { id: 7,  session_id: 1, sender_type: 3, content: '单次充电续航约 30 小时，搭配充电仓总续航可达 120 小时。支持快充，充 10 分钟可使用 3 小时。', type: 'text', created_at: '2026-05-28T10:25:00Z' },
    { id: 8,  session_id: 1, sender_type: 1, content: '人工', type: 'text', created_at: '2026-05-28T10:27:00Z' },
    { id: 9,  session_id: 1, sender_type: 0, content: '正在为您转接人工客服…', type: 'system', created_at: '2026-05-28T10:27:00Z' },
    { id: 10, session_id: 1, sender_type: 2, content: '您好，我是人工客服小张，请问还有什么我可以帮您的？', type: 'text', created_at: '2026-05-28T10:28:00Z' },
    { id: 11, session_id: 1, sender_type: 1, content: '好的，那支持无线充电吗？', type: 'text', created_at: '2026-05-28T10:28:00Z' },
    { id: 12, session_id: 1, sender_type: 2, content: '支持 Qi 无线充电，充电仓放上去就能充，很方便。', type: 'text', created_at: '2026-05-28T10:29:00Z' },
  ], total: 12 },
  ...
  'GET /admin/api/config/schemas': [
    ...
    { plugin: 'im', title: 'IM客服', fields: [
      { key: 'ai_enabled', label: '启用AI客服', type: 'switch', placeholder: '开启后新会话先由本地大模型应答，用户输入"人工"再转人工' },
      { key: 'ai_base_url', label: '大模型服务地址', type: 'text', placeholder: 'OpenAI 兼容地址，如 http://localhost:11434/v1（Ollama）' },
      { key: 'ai_api_key', label: 'API Key', type: 'password', placeholder: '本地服务可留空' },
      { key: 'ai_chat_model', label: '对话模型', type: 'text', placeholder: '如 qwen2.5:7b' },
      { key: 'ai_embed_model', label: '向量模型', type: 'text', placeholder: '如 bge-m3；留空则用关键词召回' },
      { key: 'ai_system_prompt', label: '系统提示词', type: 'textarea', placeholder: 'AI 客服人设与回答约束' },
      { key: 'ai_human_keywords', label: '转人工关键词', type: 'text', placeholder: '逗号分隔，默认：人工,转人工,人工客服,真人' },
      { key: 'ai_top_k', label: '知识库召回条数', type: 'number', placeholder: '默认 3' },
      { key: 'ai_temperature', label: '温度', type: 'number', placeholder: '0-1，默认 0.3' },
      { key: 'ai_product_search', label: '启用商品信息分析', type: 'switch', placeholder: '回答时检索在售商品的价格/库存/销量等' },
      { key: 'ai_timeout_sec', label: '请求超时(秒)', type: 'number', placeholder: '默认 30' },
      { key: 'ai_qdrant_url', label: 'Qdrant地址', type: 'text', placeholder: '如 http://localhost:6333；留空则用内存余弦/关键词召回' },
      { key: 'ai_qdrant_api_key', label: 'Qdrant API Key', type: 'password', placeholder: '自建无鉴权可留空' },
      { key: 'ai_qdrant_collection', label: 'Qdrant集合名', type: 'text', placeholder: '默认 im_knowledge' },
      { key: 'ai_score_threshold', label: '相似度阈值', type: 'number', placeholder: '0-1，默认 0 不过滤' },
      { key: 'ai_hybrid', label: '启用混合检索', type: 'switch', placeholder: '向量召回 + 关键词召回按 RRF 融合' },
      { key: 'ai_recall_k', label: '候选召回数', type: 'number', placeholder: '重排前每路候选条数，默认 4×召回条数' },
      { key: 'ai_rerank_url', label: '重排服务地址', type: 'text', placeholder: 'Cohere/Jina/TEI 兼容 /rerank，留空则不重排' },
      { key: 'ai_rerank_api_key', label: '重排 API Key', type: 'password', placeholder: '自建无鉴权可留空' },
      { key: 'ai_rerank_model', label: '重排模型', type: 'text', placeholder: '如 bge-reranker-v2-m3' },
      { key: 'ai_query_rewrite', label: '查询改写模式', type: 'select', options: [
        { label: '关闭', value: '' }, { label: 'rewrite（LLM 改写）', value: 'rewrite' },
        { label: 'hyde（生成假设回答）', value: 'hyde' }, { label: 'multi（多路变体+RRF）', value: 'multi' },
      ]},
      { key: 'ai_query_rewrite_n', label: '多路变体数', type: 'number', placeholder: '默认 3' },
  ...
  // Messages
  'GET /admin/api/messages': null as any,
...
export function matchMock(method: string, url: string, params?: Record<string, any>): { matched: boolean; data?: any } {
  ...
  // Messages with group + priority filters
  if (key === 'GET /admin/api/messages') {
    const group = String(query.group || '')
    const priority = String(query.priority || '')
    let list = messagesSource.slice()
    if (group) list = list.filter((row: any) => String(row.group || '') === group)
    if (priority) list = list.filter((row: any) => String(row.priority || '') === priority)
    const page = Math.max(1, Number(query.page || 1))
    const size = Math.max(1, Number(query.size || 20))
    const offset = (page - 1) * size
    return { matched: true, data: { list: list.slice(offset, offset + size), total: list.length, page, size } }
  }
  if (methodUpper === 'PUT' && /\/admin\/api\/messages\/\d+\/read$/.test(url)) {
    const id = Number(url.split('/')[4] || 0)
    const target = messagesSource.find((row: any) => Number(row.id || 0) === id)
    if (target) target.is_read = 1
    return { matched: true, data: null }
  }
  if (key === 'GET /admin/api/messages/unread-counts') {
    const counts: Record<string, number> = {}
    for (const msg of messagesSource) {
      if (!msg.is_read) counts[msg.group] = (counts[msg.group] || 0) + 1
    }
    return { matched: true, data: counts }
  }
